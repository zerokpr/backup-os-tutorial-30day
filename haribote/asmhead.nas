...
SCRNX	EQU		0x0ff4			; 解像度のXが入る番地
SCRNY	EQU		0x0ff6			; 解像度のYが入る番地
VRAM	EQU		0x0ff8			; グラフィックバッファの開始番地が入る番地
...
; 画面モード情報の確認（上の処理で[ES:DI]からの256バイトに画面モード情報が書き込まれる）
		CMP 	BYTE [ES:DI+0x19],8		; 色数
		JNE		scrn320
		CMP		BYTE [ES:DI+0x1b],4		; 色の指定方法。4はパレットモード
		JNE		scrn320
